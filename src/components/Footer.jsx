import { brand, contact, navLinks } from '../data/siteContent';

export default function Footer() {
  return (
    <footer className="px-5 py-12">
      <div className="mx-auto grid max-w-7xl gap-8 border-t border-white/10 pt-10 lg:grid-cols-[1.2fr_.9fr_1fr]">
        <div>
          <h2 className="text-2xl font-semibold text-[#fff6dd]">{brand.name}</h2>
          <p className="mt-3 text-[#c9c0ad]">Premium web design studio</p>
          <p className="mt-2 text-[#c9c0ad]">Cape Town, South Africa</p>
        </div>
        <nav className="flex flex-wrap gap-3" aria-label="Footer navigation">
          {navLinks.map(([label, id]) => (
            <a className="text-sm text-[#c9c0ad] hover:text-[#d8b56d]" href={`#${id}`} key={id}>{label}</a>
          ))}
        </nav>
        <div className="space-y-2 text-[#c9c0ad]">
          <a className="block hover:text-[#fff6dd]" href={contact.emailLink}>{contact.email}</a>
          <a className="block hover:text-[#fff6dd]" href={contact.phoneLink}>{contact.whatsapp}</a>
          <a className="block hover:text-[#d8b56d]" href={contact.whatsappLink}>WhatsApp Web Link Designs</a>
        </div>
      </div>
      <p className="mx-auto mt-8 max-w-7xl text-sm text-[#8f8778]">© {new Date().getFullYear()} {brand.name}. All rights reserved.</p>
    </footer>
  );
}
