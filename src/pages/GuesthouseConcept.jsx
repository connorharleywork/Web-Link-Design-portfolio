...
export default function GuesthouseConcept({ onBack }) {
  ...
  return (
    <div className="min-h-screen bg-[#071521] text-[#f8f1e4]">
      ...
      <section className="relative isolate flex min-h-[92vh] items-center overflow-hidden px-4 py-16 sm:px-5">
        ...
        <div className="mx-auto grid w-full max-w-7xl items-end gap-8 lg:grid-cols-[1.05fr_.7fr]">
          <Reveal>
            ...
            <h1 className="max-w-4xl text-5xl font-semibold leading-[0.95] tracking-[-0.05em] text-white sm:text-7xl lg:text-8xl">Wake up to the Atlantic.</h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <button onClick={() => goToEnquiry('Guesthouse Website Concept — availability enquiry demo')} className="premium-button rounded-full bg-[#d8b56d] px-7 py-4 font-bold text-[#071521]">Check Availability</button>
          ...
          <Reveal delay={0.08} className="rounded-[2rem] border border-white/15 bg-[#fbf4e7]/95 p-5 text-[#071521] shadow-2xl shadow-black/30">
            ...
            <button onClick={() => goToEnquiry('Guesthouse Website Concept — booking panel search demo')} className="mt-5 inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-full bg-[#071521] px-5 font-bold text-white"><Search size={18} /> Search Stay</button>
            <p className="mt-4 text-xs leading-5 text-[#6e6255]">Demo booking panel — connects to a direct enquiry flow for portfolio showcase purposes.</p>
          </Reveal>
        </div>
      </section>

      <main>
        <section className="px-4 py-20 sm:px-5"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><Reveal><SectionIntro eyebrow="Welcome" title="A calm coastal base with boutique warmth." text="Azure Tide Guesthouse is imagined as a peaceful beach-side retreat: soft coastal rooms, warm hospitality, a breakfast deck for unhurried mornings, and easy access to Cape Town’s coastline, restaurants and city experiences."/><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3">{guesthouseConcept.highlights.map((item)=><span key={item} className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-sm text-[#f3e5cb]">{item}</span>)}</div></Reveal><Reveal delay={0.08}><img src={guesthouseConcept.images.about.src} alt={guesthouseConcept.images.about.alt} loading="lazy" className="h-[34rem] w-full rounded-[2.5rem] object-cover shadow-2xl shadow-black/30" /></Reveal></div></section>
        ...
        <section className="bg-[#0b1f2e] px-4 py-20 sm:px-5"><div className="mx-auto max-w-7xl"><SectionIntro eyebrow="Gallery" title="A visual story of slow coastal living." text="Click any image to view the larger lightbox demo." /> <div className="columns-1 gap-4 sm:columns-2 lg:columns-3"><p className="sr-only">Gallery images use free stock photography from Unsplash.</p>{guesthouseConcept.gallery.map((image, index)=><button key={image.src} onClick={()=>setActiveImage(image)} className="group mb-4 block w-full overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 text-left"><img src={image.src} alt={image.alt} loading="lazy" className={`${index % 3 === 1 ? 'h-80' : 'h-64'} w-full object-cover transition duration-700 group-hover:scale-105`} /><span className="block px-4 py-3 text-xs text-[#bfc9c7]">Photo: Unsplash</span></button>)}</div></div></section>

        <section className="px-4 py-20 sm:px-5"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.9fr_1.1fr]"><div><SectionIntro eyebrow="Location" title="Perfectly placed for Cape Town’s coast." text="Bloubergstrand gives guests iconic Table Mountain views, long beach walks and relaxed restaurants with simple routes into Cape Town." /> <div className="grid gap-3 sm:grid-cols-2">{guesthouseConcept.locationPoints.map(p=><div key={p} className="rounded-2xl border border-white/10 bg-white/[0.055] p-4 text-[#eadfcf]">{p}</div>)}</div></div><div className="rounded-[2rem] border border-[#d8b56d]/25 bg-[radial-gradient(circle_at_30%_20%,rgba(216,181,109,.2),transparent_16rem),linear-gradient(135deg,#123047,#071521)] p-6 shadow-2xl"><div className="grid h-96 place-items-center rounded-[1.5rem] border border-white/10 bg-white/5 text-center"><div><MapPin className="mx-auto mb-4 text-[#d8b56d]" size={44}/><h3 className="text-2xl font-semibold text-white">Bloubergstrand, Cape Town</h3><p className="mt-3 text-[#cbd8d7]">Static map placeholder — no paid API required.</p><a href="https://www.google.com/maps/search/Bloubergstrand+Cape+Town" target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#d8b56d] px-5 py-3 font-bold text-[#071521]">Open in Google Maps <ExternalLink size={16}/></a></div></div></div></div></section>
